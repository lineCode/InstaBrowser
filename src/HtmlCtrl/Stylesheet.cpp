...
void Css::ParseStylesheet( const String& str, const String& baseurl, Document* doc, MediaQueryList::ptr& media )
{
	PERFMETER_BEGIN(PERF_CSSPARSE);
...
bool Css::ParseSelectors( const String& txt, Style::ptr styles, MediaQueryList::ptr& media )
{
	String Selector = txt;
	Trim(Selector);
	StringVector tokens;
	...
	{
		CssSelector::ptr selector = new CssSelector(GetObjectPool(), media);
...
{
	Sort(m_selectors, StdLess<CssSelector::ptr>( ));
}

void Css::ParseAtRule( const String& text, const String& baseurl, Document* doc, MediaQueryList::ptr& media )
{
	if(text.Mid(0, 7) == "@import")
	{
		...
		if(!tokens.IsEmpty())
		{
			...
			if(doc)
			{
				HtmlFile* doc_cont = doc->GetHtmlFile();
				if(doc_cont)
				{
					String css_text;
					String css_baseurl;
					...
					if(!css_text.IsEmpty())
					{
						MediaQueryList::ptr new_media = media;
						if(!tokens.IsEmpty())
						{
							String media_str;
	...
	} else if(text.Mid(0, 6) == "@media")
	{
		...
		if(b1 != -1)
		{
			...
			Trim(MediaType);
			MediaQueryList::ptr new_media = MediaQueryList::CreateFromString(MediaType, doc);
